  ...
  return (
    <div className="px-6 md:px-16 lg:px-24 xl:px-44 py-20 overflow-hidden">
      ...
      <div className="relative mt-6">
        ...
        <ReactPlayer url={currentTrailer.videoUrl} controls playing muted className='mx-auto max-w-full' width='960px' height='540px'/>
      </div> 
        ...
        {dummyTrailers.map((trailer)=>{
            return(
              <div key={trailer.image}  className="relative group-hover:not-hover:opacity-50 hover:-translate-y-1 duration-300 transition max-md:h-60 md:max-h-60 cursor-pointer" onClick={()=>{setCurrentTrailer(trailer)}}>
                ...
                <PlayCircleIcon strokeWidth={1.6} className="absolute top-1/4 left-1/2 w-5 md:w-8 h-5 md:h-12 transform -translate-x-1/2" />
              </div>
            )
        })}
